...
def get_all_files_in_folder(full=False):
    ...
    files = files.difference(
        set(f for f in files if any(word in f for word in banned_org_roam_files))
    )
    if not full:
...
def node_to_dict(node, file_name):
    node_dict = {
        ...
        "node_hash": calculate_sha1_hash(
            (node.body + extract_org_title(node)).encode()
        ),
    }
    return node_dict
...
def org_roam_df():
    ...
    df["text_to_encode"] = (
        "[" + df["node_hierarchy"] + "] " + df["text_to_encode"].astype(str)
    )
    return df
